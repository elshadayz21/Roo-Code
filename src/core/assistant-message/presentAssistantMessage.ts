...
export async function presentAssistantMessage(cline: Task) {
	...
	switch (block.type) {
		...
		case "tool_use": {
			...
			switch (block.name) {
				// === GATEKEEPER: Phase 1 Reasoning Loop ===
				// Block side-effect tools until the agent has declared an active intent.
				// Safe tools (read-only, meta, or the intent tool itself) are exempted.
				case "write_to_file":
				case "apply_diff":
				case "execute_command":
				case "insert_content":
				...
				case "new_task": {
					if (!cline.activeIntentId) {
						...
						console.warn(
							`[Gatekeeper] Blocked tool "${block.name}" — no activeIntentId on task ${cline.taskId}`,
						)
						cline.pushToolResultToUserContent({
							type: "tool_result",
							tool_use_id: sanitizeToolUseId(toolCallId),
			...
			// === TOOL DISPATCH ===
			// Only execute tools that passed the Gatekeeper (or were exempted).
			// Check: if the tool was blocked above, a tool_result was already pushed, so skip.
			const wasBlockedByGatekeeper =
		...
		} // Close if (!wasBlockedByGatekeeper)
	}
