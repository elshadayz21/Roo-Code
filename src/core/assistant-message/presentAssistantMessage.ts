...
export async function presentAssistantMessage(cline: Task) {
	...
	switch (block.type) {
		...
		case "tool_use": {
			...
			// === MIDDLEWARE / HOOK ENGINE INTERCEPTION ===
			const hookResult = await globalHookEngine.runPreHooks({
				task: cline,
				block: block as ToolUse,
				toolCallId,
			})
			...
			if (hookResult.blocked) {
				cline.pushToolResultToUserContent({
					type: "tool_result",
					tool_use_id: sanitizeToolUseId(toolCallId),
					content: hookResult.errorMessage || "Tool blocked by hook",
					is_error: true,
				})
				break
			}

			if (hookResult.transformedBlock) {
				block = hookResult.transformedBlock
			}
